/** Typed copy for all portfolio controls, metrics and empty/error states. */
export declare const NS = "fundPortfolio";
export declare const en: {
    title: string;
    sidebarLabel: string;
    subtitle: string;
    today: string;
    confirmed: string;
    estimated: string;
    missing: string;
    market: string;
    cost: string;
    floating: string;
    covered: string;
    accounts: string;
    allAccounts: string;
    addAccount: string;
    accountName: string;
    rename: string;
    delete: string;
    addHolding: string;
    editHolding: string;
    edit: string;
    refresh: string;
    refreshing: string;
    save: string;
    cancel: string;
    lookup: string;
    code: string;
    shares: string;
    costPrice: string;
    account: string;
    name: string;
    nav: string;
    estimate: string;
    noData: string;
    pending: string;
    baseline: string;
    review: string;
    unavailable: string;
    reference: string;
    recent: string;
    empty: string;
    emptyHint: string;
    disclaimer: string;
    source: string;
    backup: string;
    export: string;
    import: string;
    preview: string;
    replace: string;
    merge: string;
    importWarning: string;
    confirm: string;
    deleteQuestion: string;
    conflict: string;
    privacy: string;
    moneyYield: string;
    annual: string;
    money: string;
    qdii: string;
    navFund: string;
    amount: string;
    date: string;
    priceDate: string;
    estimateTime: string;
    referenceDate: string;
    fetched: string;
    error: string;
    duplicateHolding: string;
    chooseAccount: string;
    validated: string;
    confirmFund: string;
    close: string;
    viewData: string;
    details: string;
    zeroCost: string;
    loading: string;
    importReady: string;
    noConflicts: string;
    holdings: string;
    partial: string;
    stale: string;
    rate: string;
    quoteStatus: string;
    quoteDetails: string;
    estimateUnavailable: string;
    estimatePending: string;
    transfer: string;
    chooseFile: string;
    readingFile: string;
    jsonFileOnly: string;
    fileTooLarge: string;
    invalidJson: string;
    fileReadError: string;
    accountHelp: string;
    allocation: string;
    allocationHelp: string;
    targetRatio: string;
    currentRatio: string;
    targetTotal: string;
    remaining: string;
    configured: string;
    allocationUnconfigured: string;
    allocationInvalid: string;
    allocationMarketIncomplete: string;
    allocationReady: string;
    theoreticalBuy: string;
    theoreticalSell: string;
    theoreticalHold: string;
    estimatedShares: string;
    allocationDisclaimer: string;
};
export type PortfolioKey = keyof typeof en;
export declare const zh: Record<PortfolioKey, string>;
