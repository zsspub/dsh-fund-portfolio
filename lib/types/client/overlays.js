...
export function PortfolioModal({ title, closeLabel, busy, onClose, children, alert = false, initialFocusId }) {
    ...
    return _jsx(Modal, { open: true, title: title, closeLabel: closeLabel, onClose: () => closeRef.current(), className: "fp-modal", contentClassName: "fp-modal-content", children: _jsx("div", { ref: contentRef, children: children }) });
}
    ...
    return _jsx("span", { ref: anchorRef, onKeyDown: event => {
            if (open && event.key === 'Tab') {
                ...
                if (id === 'import')
                    ...
                    onExport();
            }, anchor: _jsxs(Button, { variant: "outline", "aria-disabled": disabled, "aria-haspopup": "menu", "aria-expanded": open, onClick: () => { if (!disabled) {
                    setFocusLast(false);
                    setOpen(!open);
                } }, onKeyDown: event => {
