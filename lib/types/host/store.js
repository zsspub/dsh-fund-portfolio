...
import { codeSchema, decimalText, nameSchema, parseAccount, parseBackup, parseFund, parseHolding, positiveText, signedDecimalText, targetRatioText, } from "./validation.js";
import { z } from 'zod';
...
export class PortfolioStore {
    ...
    addHolding(input) {
        ...
        const holding = { ...input, id: randomUUID(), targetRatio: null, version: 1 };
    ...
    updateAllocation(input) {
        ...
        const total = input.allocations.reduce((sum, allocation) => sum.add(targetRatioText.parse(allocation.targetRatio)), new Decimal(0));
        if (!total.eq(100))
