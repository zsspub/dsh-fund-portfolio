...
import { backupSchema, codeSchema, decimalText, nameSchema, parseAccount, parseFund, parseHolding, positiveText, signedDecimalText } from "./validation.js";
import { z } from 'zod';
...
export class PortfolioStore {
    ...
    constructor(path, busyTimeoutMs) {
        if (!isAbsolute(path))
            throw new Error('databasePath must be absolute');
        ...
        try {
            ...
            if (version > 1)
                throw new Error('Database schema is newer than this plugin');
    ...
    createAccount(name) {
        const account = { id: randomUUID(), name: nameSchema.parse(name), version: 1 };
    ...
    editAccount(input) {
        ...
        if (result.changes !== 1)
            throw new Error('Account changed or no longer exists; refresh first');
        return this.account(input.id);
    }
    deleteAccount(input) {
        if (this.holdings().some(item => item.accountId === input.id))
            throw new Error('Remove account holdings before deleting the account');
    ...
    checkHolding(input) {
        ...
        if (!fund)
            throw new Error('Look up and confirm the fund before adding it');
    ...
    addHolding(input) {
        ...
        if (duplicate) {
            throw new RemoteError('fund-portfolio/duplicate-holding', 'This account already has the fund; edit the existing holding instead', { accountId: input.accountId, fundCode: input.fundCode });
        }
        const holding = { ...input, id: randomUUID(), version: 1 };
        this.database.prepare('INSERT INTO holdings VALUES (?,?,?,?,?,?)')
            .run(holding.id, holding.accountId, holding.fundCode, holding.shares, holding.costPrice, holding.version);
        return holding;
    }
    editHolding(input) {
        this.checkHolding(input);
        const result = this.database.prepare('UPDATE holdings SET account_id=?,fund_code=?,shares=?,cost_price=?,version=version+1 WHERE id=? AND version=?')
            .run(input.accountId, input.fundCode, input.shares, input.costPrice, input.id, input.version);
        if (result.changes !== 1)
            throw new Error('Holding changed or no longer exists; refresh first');
    ...
    saveQuote(quote) {
        ...
        this.transaction(() => {
            ...
            for (const nav of quote.navs)
                write.run(quote.code, nav.date, 'nav', JSON.stringify(nav));
            if (quote.estimate)
                write.run(quote.code, quote.estimate.date, 'estimate', JSON.stringify(quote.estimate));
        });
    }
    backup() {
        return {
            format: 1, accounts: this.accounts(), holdings: this.holdings(),
            funds: this.database.prepare('SELECT json FROM funds ORDER BY code').all().map(row => parseFund(JSON.parse(String(row.json)))),
        };
    }
    readBackup(json) {
        if (Buffer.byteLength(json, 'utf8') > 20 * 1024 * 1024)
            throw new Error('Backup exceeds 20 MiB');
        ...
        if (!unique(backup.accounts.map(item => item.id)) || !unique(backup.accounts.map(item => item.name))
            ...
            || !unique(backup.holdings.map(item => `${item.accountId}/${item.fundCode}`)))
            throw new Error('Duplicate backup records');
        for (const holding of backup.holdings) {
            const fund = backup.funds.find(item => item.code === holding.fundCode);
            if (!backup.accounts.some(item => item.id === holding.accountId) || !fund)
                throw new Error('Backup has missing references');
            if (fund.kind === 'money' && !/^1(?:\.0+)?$/.test(holding.costPrice))
                throw new Error('Invalid money fund unit cost');
        }
        return backup;
    }
    preview(json) {
    ...
    import(input) {
        this.transaction(() => {
            ...
            if (preview.previewToken !== input.previewToken)
                throw new Error('Backup or holdings changed; preview again');
            if (input.mode === 'merge' && preview.conflicts.length)
                throw new Error('Merge has conflicts; use an explicitly confirmed replacement');
            ...
            if (input.mode === 'replace')
                this.database.exec('DELETE FROM holdings; DELETE FROM accounts;');
            ...
            for (const holding of backup.holdings)
                this.database.prepare('INSERT INTO holdings VALUES (?,?,?,?,?,?)')
                    .run(holding.id, holding.accountId, holding.fundCode, holding.shares, holding.costPrice, Math.max(holding.version, oldHoldings.get(holding.id) ?? 0) + 1);
        });
    }
