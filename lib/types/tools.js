...
import { z } from 'zod';
import { codeSchema, decimalText, idSchema, nameSchema, positiveText, targetRatioText, versionSchema, } from "./host/validation.js";
...
export function apply(ctx) {
    ...
    register('fund_allocation_update', '原子设置一个账户全部现有基金的目标占比。必须包含该账户每条持仓的最新版本，目标占比为最多两位小数且合计精确为 100%。', {
        ...
        allocations: input.allocations.map(allocation => ({ ...allocation, id: allocation.id })),
    }, exec.signal));
