import { a as nameSchema, c as parseFund, d as priorCalendarDate, f as signedDecimalText, l as parseHolding, m as todayAt, n as dateString, o as parseAccount, p as targetRatioText, r as decimalText, s as parseBackup, t as codeSchema, u as positiveText } from "./validation-5OI2sbXh.js";
...
var PortfolioStore = class {
	...
	editHolding(input) {
		...
		const targetRatio = previous.accountId === input.accountId && previous.fundCode === input.fundCode ? previous.targetRatio : null;
		if (this.database.prepare(`
      UPDATE holdings SET account_id=?,fund_code=?,shares=?,cost_price=?,target_ratio=?,version=version+1
      WHERE id=? AND version=?
    `).run(input.accountId, input.fundCode, input.shares, input.costPrice, targetRatio, input.id, input.version).changes !== 1) throw new Error("Holding changed or no longer exists; refresh first");
	...
	updateAllocation(input) {
		...
		if (current.length === 0 || input.allocations.length !== current.length) throw new Error("Allocation must include every holding in the account");
		const currentIds = new Set(current.map((holding) => holding.id));
		if (new Set(input.allocations.map((allocation) => allocation.id)).size !== input.allocations.length || input.allocations.some((allocation) => !currentIds.has(allocation.id))) throw new Error("Allocation holdings do not match the account");
		if (!input.allocations.reduce((sum, allocation) => sum.add(targetRatioText.parse(allocation.targetRatio)), new Decimal(0)).eq(100)) throw new Error("Target allocation must total exactly 100%");
		return this.transaction(() => {
			const update = this.database.prepare(`
        UPDATE holdings SET target_ratio=?,version=version+1
        WHERE id=? AND account_id=? AND version=?
      `);
			for (const allocation of input.allocations) if (update.run(allocation.targetRatio, allocation.id, input.accountId, allocation.version).changes !== 1) throw new Error("Holding changed or no longer exists; refresh first");
	...
	import(input) {
        ...
        INSERT INTO holdings (id,account_id,fund_code,shares,cost_price,target_ratio,version)
        VALUES (?,?,?,?,?,?,?)
      `).run(holding.id, holding.accountId, holding.fundCode, holding.shares, holding.costPrice, holding.targetRatio, Math.max(holding.version, oldHoldings.get(holding.id) ?? 0) + 1);
		});
	}
...
function allocationFor(accountId, holdings) {
	...
	const marketComplete = holdings.length > 0 && holdings.every((row) => row.marketValue !== null && new Money(row.marketValue).gt(0));
	const totalMarketValue = marketComplete ? holdings.reduce((total, row) => total.add(row.marketValue), new Money(0)) : new Money(0);
	if (marketComplete && totalMarketValue.gt(0)) for (const row of holdings) row.currentRatio = new Money(row.marketValue).div(totalMarketValue).mul(100).toFixed();
	const base = {
	...
	if (configured.length !== holdings.length || holdings.length === 0) return {
		...base,
		status: "unconfigured"
	};
	if (!targetTotal.eq(100)) return {
		...base,
		status: "invalid-target-total"
	};
	if (!marketComplete || !totalMarketValue.gt(0)) return {
		...base,
		status: "market-incomplete"
	};
	let buyAmount = new Money(0);
	let sellAmount = new Money(0);
	for (const row of holdings) {
		const marketValue = new Money(row.marketValue);
		const delta = totalMarketValue.mul(row.holding.targetRatio).div(100).sub(marketValue);
...
function summarize(date, accounts, holdings, refreshIntervalMs, accountId) {
	const sum = (values) => {
...
let FundPortfolioService = (() => {
	...
	let _allocationUpdate_decorators;
	...
	return class FundPortfolioService extends _classSuper {
		static {
			...
			_allocationUpdate_decorators = [Remote];
			...
			__esDecorate(this, null, _allocationUpdate_decorators, {
				kind: "method",
				name: "allocationUpdate",
				static: false,
				private: false,
				access: {
					has: (obj) => "allocationUpdate" in obj,
					get: (obj) => obj.allocationUpdate
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		async allocationUpdate(request, signal) {
		...
		async summary(request, signal) {
			return remoteOperation("portfolio.summary", signal, async () => {
				...
				}), this.config.refreshIntervalMs, request.accountId);
			});
		}
