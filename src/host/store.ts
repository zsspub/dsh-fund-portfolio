...
import type {
  Account, AccountEdit, AccountId, AccountRef, Backup, Fund, Holding, HoldingEdit, HoldingId,
  HoldingInput, HoldingRef, ImportCommit, ImportPreview, Quote,
} from '../types.ts'
import { backupSchema, codeSchema, decimalText, nameSchema, parseAccount, parseFund, parseHolding, positiveText, signedDecimalText } from './validation.ts'
import { z } from 'zod'
...
export class PortfolioStore {
  ...
  addHolding(input: HoldingInput): Holding {
    ...
    const holding = { ...input, id: randomUUID() as HoldingId, version: 1 }
  ...
  private readBackup(json: string): Backup {
    if (Buffer.byteLength(json, 'utf8') > 20 * 1024 * 1024) throw new Error('Backup exceeds 20 MiB')
    const backup = backupSchema.parse(JSON.parse(json)) as Backup
  ...
  import(input: ImportCommit): void {
    this.transaction(() => {
      ...
      for (const holding of backup.holdings) this.database.prepare('INSERT INTO holdings VALUES (?,?,?,?,?,?)')
